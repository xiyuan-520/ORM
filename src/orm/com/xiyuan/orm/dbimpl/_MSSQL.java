 ...
 */
public class _MSSQL implements ORMPolicy
{
    private ORMServer server;
    
    public _MSSQL(ORMServer server)
    {
        this.server = server;
    }
    ...
    {
        return Strings.format(T_EXISTS_MSSQL, tableName);
    }
    ...
    public String toAlertColumnAdd(String table, String column, String columnType, boolean notNull)
    {
        return new StringBuilder()
            ...
            .append(" add ").append(column.toUpperCase()).append(" ").append(columnType)
    ...
    public String toColumnType(String type)
    {
        ...
        {
        //基本类型
        case Z_ORM_BOOLEAN_INT:     return "bit";
        case Z_ORM_BYTE_INT:        return "tinyint";
        case Z_ORM_SHORT_INT:       return "smallint";
        ...
        //二进制字节数组
        case Z_ORM_BINARY_INT:  return "image";
    ...
    public String toPageSQL(String fieldSQL, String tableName, String whereSQL, String orderbySQL, String groupbySQL, int maxNum, int pageNo)
    {
        ...
        {
            return new StringBuilder()
                .append("select top ").append(maxNum).append(" ").append(fieldSQL)
                .append(" from ").append(tableName).append(whereSQL).append(orderbySQL)
                .toString();
        }
        else
        {
            return new StringBuilder()
                .append("select top ").append(maxNum).append(" identity(int,1,1) as idkey_, ").append(fieldSQL).append(" into _temp_zhiqim_page from ").append(tableName).append(whereSQL).append(orderbySQL).append(groupbySQL)
                .append(" select * from _temp_zhiqim_page where idkey_ >= #minNum#")
                .append(" drop table _temp_zhiqim_page")
                .toString();
        }
    }
    ...
    {
        if (pageNo == 1)
            return new StringBuilder("select top ").append(maxNum).append(" * from ").append(innerTableSQL).toString();
        else
            return new StringBuilder("select top ").append(maxNum).append(" identity(int,1,1) as idkey_, * into _temp_zhiqim_page from ").append(innerTableSQL)
                .append(" select * from _temp_zhiqim_page where idkey_ >= #minNum#")
                .append(" drop table _temp_zhiqim_page")
                .toString();
    }
    ...
    {//利用存储过程
        ...
        server.sql().execute(new StringBuilder("sp_rename '").append(table.toUpperCase()).append(".").append(column.toUpperCase()).append("', '").append(newColumn.toUpperCase()).append("'").toString());
    }
    ...
    {//标准SQL92
        String sql = new StringBuilder()
            .append("alter table ").append(table).append(" alter column ").append(column.toUpperCase()).append(" ").append(newColumnType).append(newNotNull?" not null":" default null")
            .toString();
    ...
    {//分成两句处理
        String sql = new StringBuilder()
            .append("sp_rename '").append(table.toUpperCase()).append(".").append(column.toUpperCase()).append("', '").append(newColumn.toUpperCase()).append("'").append(";")
            .append("alter table ").append(table.toUpperCase()).append(" alter column ").append(column.toUpperCase()).append(" ").append(newColumnType).append(newNotNull?" not null":" default null").append(";")
            .toString();
